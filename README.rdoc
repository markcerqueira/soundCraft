...
4. Play the LOrkCraft Shipyard map from the Galaxy Editor or from Starcraft 2.

== FILES
1. *lorkCraftShipyard.SC2Map* - the Antiga Shipyard map (forced cross-spawns at the 5 and 11 o'clock positions) with the custom data-collecting triggers applied to it; for user focus the bottom-left coordinates are (30, 18) and the top-right coordinates are (138, 146)
...
== Todos
* Timestamp all bank parsing and log info so a playthrough can be "recreated" through the debug logs. This will be useful for testing the audio component of the system without needing to play games over and over again.
* Resilient thing that restarts the Ruby script if it fails (in case something goes wrong during the performance)
* Check out other Ruby OSC libraries
* Normalize (x, y) coordinates to more natural system
